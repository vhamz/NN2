...
// Сохраняем те же пиксели (histogram invariant)
...
// Smoothness — локальная плавность
...
function studentLoss(yTrue, yPred) {
  return tf.tidy(() => {
    ...
    const lossSmooth = smoothness(yPred).mul(0.12);

    const lossDir = directionX(yPred).mul(0.35);
